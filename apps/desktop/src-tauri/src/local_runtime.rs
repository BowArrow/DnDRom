...
impl ManagedProcessJob {
    fn new() -> Result<Self, String> {
        ...
        Ok(Self { handle: Mutex::new(Some(handle)) })
    }

    fn assign_child(&self, child: &Child) -> Result<(), String> {
        let handle = self.handle.lock().map_err(|_| "Local AI process group lock failed".to_string())?;
        ...
        if assigned == 0 { return Err("Could not attach the local AI engine to DnDRom's process group".into()); }
        Ok(())
    }

    fn assign_pid(&self, pid: u32) -> Result<(), String> {
        let process = unsafe { OpenProcess(PROCESS_SET_QUOTA | PROCESS_TERMINATE, 0, pid) };
        if process.is_null() { return Err("Could not reopen the existing local AI engine".into()); }
        let result = self.handle.lock().map_err(|_| "Local AI process group lock failed".to_string()).and_then(|handle| {
            let job = handle.ok_or("The local AI process group is already closed")?;
    ...
    fn shutdown(&self) {
        let Ok(mut handle) = self.handle.lock() else { return; };
        let Some(job) = handle.take() else { return; };
        unsafe {
...
impl ManagedProcessJob {
    ...
    fn assign_child(&self, _child: &Child) -> Result<(), String> { Ok(()) }
    fn shutdown(&self) {}
}
...
impl Default for LocalRuntimeState {
    fn default() -> Self { Self::new().expect("could not initialize the local AI process group") }
}
...
impl Drop for LocalRuntimeState {
    fn drop(&mut self) {
        ...
        if Arc::strong_count(&self.process_job) == 1 { self.process_job.shutdown(); }
    }
}
...
const PROP_SANA_MODELS: &[DownloadAsset] = &[
    ...
    DownloadAsset { name: "Sana DC-AE image decoder", url: "https://huggingface.co/mit-han-lab/dc-ae-f32c32-sana-1.1/resolve/main/model.safetensors?download=true", relative_path: "vae/dc-ae-f32c32-sana-1.1.safetensors", size: 1_249_046_372, sha256: "01a99a6d08b8e1d54bf9af28cab521432cdeee372720e84c38bac8b8abe27ba6" },
];

const PROP_SANA_NODES: &[NodeArchive] = &[
    NodeArchive { name: "ComfyUI_ExtraModels", repo: "lawrence-cj/ComfyUI_ExtraModels", commit: "09e895cf35384979e0bb9a1ea01fe8f3db913516", size: 3_187_651, sha256: "7e439854da41c3aef5282f65733bfc0836f1d8fded59539c56e338986fc3e01e" },
];
...
fn runtime_base(app: &AppHandle) -> Result<PathBuf, String> {
    let path = app
        .path()
        .app_local_data_dir()
...
fn feature_total(feature: RuntimeFeature, archive: RuntimeArchive) -> u64 {
    archive.size
        ...
        + if feature == RuntimeFeature::World { BRUSH.size } else { 0 }
}
...
pub async fn local_runtime_status(
    ...
    tauri::async_runtime::spawn_blocking(move || status_inner(&app, feature))
        .await
        .map_err(|error| error.to_string())?
}

fn status_inner(app: &AppHandle, feature: RuntimeFeature) -> Result<RuntimeStatus, String> {
...
pub async fn ensure_local_runtime(
    ...
    tauri::async_runtime::spawn_blocking(move || ensure_inner(&app, &manager, feature, vram_reserve_gb.unwrap_or(0.75)))
        .await
        .map_err(|error| error.to_string())?
}
...
fn ensure_inner(
    ...
    let node_setup_marker = portable_root(app)?.join("ComfyUI/custom_nodes").join(match feature {
    ...
    let node_setup_needed = (!nodes(feature).is_empty() && !node_setup_marker.exists()) || runtime_node_needed;
...
fn install_models(app: &AppHandle, feature: RuntimeFeature, total: u64) -> Result<(), String> {
    ...
    for asset in models(feature) {
        ...
        if feature == RuntimeFeature::PropImageLite
            && asset.relative_path == "text_encoders/models-efficient-large-model--gemma-2-2b-it/model.safetensors"
            && !target.exists()
        {
...
fn install_world_nodes(app: &AppHandle, state: &LocalRuntimeState, feature: RuntimeFeature, total: u64) -> Result<(), String> {
    let root = portable_root(app)?;
    ...
    let mut completed = runtime_archive().size + models(feature).iter().map(|asset| asset.size).sum::<u64>();
    for node in nodes(feature) {
    ...
    if feature == RuntimeFeature::PropImageLite {
        ...
        if patched != source {
            fs::write(&gemma_node, patched)
                .map_err(|error| format!("Could not finish the offline Sana text encoder setup: {error}"))?;
        }
    }
    ...
    for node in nodes(feature) {
        ...
        if node.name == "ComfyUI-UniRig" {
            ...
            if installer.exists() {
                ...
                installer_command.arg(&installer).current_dir(custom_nodes.join(node.name));
    ...
    if feature == RuntimeFeature::PropImageLite {
        ...
        let status = run_managed_command(state, &mut dependency)
            .map_err(|error| format!("Could not install the Sana image decoder dependency: {error}"))?;
        if !status.success() {
        ...
        let status = run_managed_command(state, &mut verify)
            .map_err(|error| format!("Could not verify the Sana image decoder dependency: {error}"))?;
        if !status.success() {
            return Err("The Sana image decoder dependency was installed but could not be imported".to_string());
        }
    }
...
fn install_brush(app: &AppHandle, feature: RuntimeFeature, total: u64) -> Result<(), String> {
    let base = runtime_base(app)?;
...
fn run_managed_command(state: &LocalRuntimeState, command: &mut Command) -> std::io::Result<ExitStatus> {
...
#[cfg(target_os = "windows")]
fn adopt_trusted_orphaned_server(app: &AppHandle, state: &LocalRuntimeState) -> Result<bool, String> {
    ...
    let Some((pid, executable)) = value.trim().split_once('\t') else { return Ok(false); };
    ...
    if !executable_is_inside_runtime(&executable, &trusted_root) { return Ok(false); }
    let pid = pid.trim().parse::<u32>().map_err(|_| "The local creation engine reported an invalid process id".to_string())?;
    state.process_job.assign_pid(pid)?;
    Ok(true)
}

#[cfg(not(target_os = "windows"))]
fn adopt_trusted_orphaned_server(_app: &AppHandle, _state: &LocalRuntimeState) -> Result<bool, String> { Ok(false) }

#[cfg(target_os = "windows")]
fn stop_trusted_orphaned_server(app: &AppHandle) -> Result<bool, String> {
    let script = r#"$connection = Get-NetTCPConnection -LocalPort 8189 -State Listen -ErrorAction SilentlyContinue | Where-Object { $_.LocalAddress -in @('127.0.0.1', '::1') } | Select-Object -First 1; if ($connection) { $process = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $connection.OwningProcess); [Console]::Out.Write($connection.OwningProcess.ToString() + "`t" + $process.ExecutablePath) }"#;
    let output = hidden_command(Path::new("powershell.exe"))
        .args(["-NoProfile", "-NonInteractive", "-Command", script])
        .output()
...
#[cfg(not(target_os = "windows"))]
fn stop_trusted_orphaned_server(_app: &AppHandle) -> Result<bool, String> {
    Ok(false)
}

fn stop_managed_server(app: &AppHandle, state: &LocalRuntimeState) -> Result<(), String> {
    let mut child = state
        .child
        .lock()
...
fn start_server(
    ...
    if server_ready() {
        let has_owned_handle = state.child.lock().map(|slot| slot.is_some()).unwrap_or(false);
        if !has_owned_handle {
    ...
    if let Ok(mut slot) = state.child.lock() {
        if let Some(mut previous) = slot.take() {
            match previous.try_wait() {
                Ok(Some(_)) => { let _ = previous.wait(); }
                Ok(None) => {
                    let _ = previous.kill();
                    let _ = previous.wait();
                }
                Err(_) => { let _ = previous.kill(); }
            }
        }
    }
    let root = portable_root(app)?;
...
fn latest_complete_world_dataset(comfy_root: &Path, max_age: Duration) -> Result<Option<PathBuf>, String> {
    ...
    for entry in fs::read_dir(&output).map_err(|error| error.to_string())?.flatten() {
        let root = entry.path();
        ...
        if !required.iter().all(|name| sparse.join(name).metadata().is_ok_and(|metadata| metadata.len() > 0)) {
            continue;
        }
        let modified = sparse.join("images.bin").metadata()
...
fn latest_trained_world(runtime_root: &Path, max_age: Duration) -> Result<Option<PathBuf>, String> {
    ...
    let cutoff = SystemTime::now().checked_sub(max_age).unwrap_or(UNIX_EPOCH);
    let mut newest: Option<(SystemTime, PathBuf)> = None;
    for entry in fs::read_dir(&generated).map_err(|error| error.to_string())?.flatten() {
...
pub fn find_latest_local_world_dataset(app: AppHandle) -> Result<Option<String>, String> {
    ...
    Ok(latest_complete_world_dataset(&comfy, Duration::from_secs(12 * 60 * 60))?
        .map(|path| path.to_string_lossy().to_string()))
}

#[tauri::command]
pub fn find_latest_local_trained_world(app: AppHandle) -> Result<Option<TrainedWorld>, String> {
    Ok(latest_trained_world(&runtime_base(&app)?, Duration::from_secs(12 * 60 * 60))?
        .map(|path| TrainedWorld {
...
pub async fn train_local_world(
    ...
    let manager = state.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
        train_world_inner(&app, &manager, &dataset_path, total_steps, max_splats, max_frames)
    })
        .await
        .map_err(|error| error.to_string())?
}
...
fn training_step_from_line(line: &str, total_steps: u32) -> Option<u32> {
    ...
    let digits = tail.trim_start_matches(|character: char| !character.is_ascii_digit())
        .chars()
        .take_while(|character| character.is_ascii_digit())
        .collect::<String>();
    digits.parse::<u32>().ok().filter(|step| *step <= total_steps)
}

fn validation_loss_from_line(line: &str) -> Option<f64> {
    let lower = line.to_ascii_lowercase();
    if !lower.contains("validation") && !lower.contains("val_loss") && !lower.contains("val loss") { return None; }
    let marker = lower.find("loss")?;
    lower[marker + 4..]
        .trim_start_matches(|character: char| !character.is_ascii_digit() && character != '.' && character != '-')
        .split(|character: char| character.is_whitespace() || character == ',' || character == ';')
        .next()?
        .parse::<f64>().ok().filter(|value| value.is_finite())
}
...
fn stage_brush_colmap_dataset(dataset: &Path, output: &Path) -> Result<PathBuf, String> {
    ...
    for entry in fs::read_dir(dataset.join("images")).map_err(|error| error.to_string())?.flatten() {
...
fn ply_vertex_count(path: &Path) -> Option<u32> {
    ...
    text.lines().find_map(|line| line.strip_prefix("element vertex ")?.trim().parse().ok())
}
...
fn train_world_inner(
    ...
    let progress_reader = |reader: Box<dyn Read + Send>, progress_app: AppHandle, early_stop: Arc<Mutex<BrushEarlyStopState>>| {
        thread::spawn(move || {
            for line in BufReader::new(reader).lines().map_while(Result::ok) {
                if let Some(step) = training_step_from_line(&line, total_steps) {
                    if let Ok(mut state) = early_stop.lock() { state.current_step = state.current_step.max(step); }
                    emit(
                        &progress_app,
                        RuntimeFeature::World,
                        "training",
                ...
                if let Some(loss) = validation_loss_from_line(&line) {
                    if let Ok(mut state) = early_stop.lock() {
                        let improved = state.best_loss.is_none_or(|best| loss < best - best.abs().max(1e-8) * 0.0005);
                        if improved { state.best_loss = Some(loss); state.last_improved_step = state.current_step; }
                    }
                }
            }
        })
    };
    let stdout_thread = stdout.map(|reader| progress_reader(Box::new(reader), app.clone(), early_stop.clone()));
    let stderr_thread = stderr.map(|reader| progress_reader(Box::new(reader), app.clone(), early_stop.clone()));
    let export_path = output.join("dndrom-world.ply");
    let mut stopped_early = false;
    let status = loop {
        if let Some(status) = trainer.try_wait().map_err(|error| format!("Could not wait for the local world trainer: {error}"))? { break status; }
        let plateau = early_stop.lock().ok().is_some_and(|state| state.best_loss.is_some() && state.current_step >= 3_000 && state.current_step.saturating_sub(state.last_improved_step) >= 250);
        if plateau && export_path.exists() {
            emit(app, RuntimeFeature::World, "training", total_steps as u64, total_steps as u64, "Validation loss plateaued for 250 steps; preserving the latest completed export");
            trainer.kill().map_err(|error| format!("Could not stop the converged world trainer: {error}"))?;
            stopped_early = true;
            break trainer.wait().map_err(|error| format!("Could not finish the converged world trainer: {error}"))?;
        }
...
mod tests {
    ...
    fn managed_server_reserves_vram_headroom() {
        ...
        let reserve_index = args.iter().position(|value| value == "--reserve-vram").unwrap();
        assert_eq!(args.get(reserve_index + 1).map(String::as_str), Some("1.25"));
        let clamped = server_args(99.0);
        let reserve_index = clamped.iter().position(|value| value == "--reserve-vram").unwrap();
        assert_eq!(clamped.get(reserve_index + 1).map(String::as_str), Some("4.00"));
    }
    ...
    fn world_decoder_evicts_diffusion_weights_before_bounded_decode() {
        ...
        assert!(unload < decode, "WAN weights must be evicted before VAE decode");
    ...
    fn brush_progress_parser_reads_steps_without_treating_loss_as_progress() {
        assert_eq!(training_step_from_line("train step 240/3500 loss 0.021", 3500), Some(240));
        assert_eq!(training_step_from_line("iteration 91: refine", 3500), Some(91));
        assert_eq!(training_step_from_line("loss 0.021", 3500), None);
        assert_eq!(training_step_from_line("step 9000/3500", 3500), None);
        assert_eq!(validation_loss_from_line("validation step 3000 loss 0.0215"), Some(0.0215));
        assert_eq!(validation_loss_from_line("train step 3000 loss 0.0215"), None);
    }
