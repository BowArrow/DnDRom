...
describe("bundled local Sana workflow", () => {
  it("matches the pinned ExtraModels node contract", () => {
    ...
    expect(workflow["2"].inputs).toMatchObject({ model_name: "Efficient-Large-Model/gemma-2-2b-it", device: "cpu", dtype: "FP32" });
    expect(workflow["3"].inputs).toMatchObject({ vae_name: "dc-ae-f32c32-sana-1.1-diffusers.safetensors", vae_type: "dcae-f32c32-sana-1.1-diffusers", dtype: "BF16" });
