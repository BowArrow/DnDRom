"""Render the reviewed canonical coastal report; never generate/rewrite its text."""
...
work = ROOT / "artifacts/water-research-025"
...
def furniture(canvas, doc):
    ...
    canvas.drawString(46, 30, "DnDRom | Coastal water research | Implementation specification, not release acceptance")
    canvas.drawRightString(549, 30, str(doc.page))
    canvas.restoreState()

output = ROOT / "docs/research/DnDRom-Coastal-Water-Research.pdf"
SimpleDocTemplate(str(output), pagesize=(595, 842), leftMargin=46, rightMargin=46, topMargin=43, bottomMargin=59,
                  title="Coastal water that behaves like a beach", author="DnDRom engineering").build(story, onFirstPage=furniture, onLaterPages=furniture)
